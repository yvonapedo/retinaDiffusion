...
class GaussianDiffusionTrainer(nn.Module):
    ...
    def forward(self, x_0, image, entropy=None):
        """
        Algorithm 1.
        """
        ...
        if entropy != None:
            seg_pred, noisy_pred = self.model(x_t, t, image, entropy)
        else:
...
class GaussianDiffusionSampler(nn.Module):
    ...
    def forward(self, x_T, image, info):
        ...
        for _, time_step in enumerate(tqdmT):
            ...
            #     torch.save(x_t, 
            #             os.path.join("./choose_retina/diffusion_step", f"noise_{time_step}.pt"))
            #     torch.save(seg_result, 
            #             os.path.join("./choose_retina/diffusion_step", f"seg_{time_step}.pt"))
